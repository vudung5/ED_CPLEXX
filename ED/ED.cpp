...
int LoadCoeff = 2; // Load coefficient
...
for (size_t j = 0; j < MaintenanceData.data.size(); ++j) {
	...
	GridMaintenance.push_back(static_cast<int>(MaintenanceData.data[j][NumDG + NumBESS + 5]));
	// +3=SC1;   +4=SC2;   +5=SC3; 
}
...
{
	GridBuyCost.push_back(Griddata.data[i][4]);         // GridBuyCost
...
{
	P_RE_PV_ava.push_back(REdata.data[i][4]); // P_RE_PV
	P_RE_Wind_ava.push_back(REdata.data[i][3]); // P_RE_Wind
}
// ---------------------------------UC DATA-----------------------------------

string UCpath = Output_path + "\\Output_Microgrid_UC.csv";
...
for (int t = 0; t < 1440; ++t)
{
	...
	P_RE_PV_Meas.push_back(MEASUR_DATA.data[t][1]); // PV_Meas
	P_RE_Wind_Meas.push_back(MEASUR_DATA.data[t][0]); // WTG_Meas
}

//---------------------------------ED DATA PREPROCESSING-----------------------------------

const int ED_min = 362;       // <-- AT minute 362 of day
...
/* Load forcast*/
std::vector<double> Load15_fc1(P_Load_Meas1.begin(), P_Load_Meas1.end());
std::vector<double> Load15_fc2(P_Load_Meas2.begin(), P_Load_Meas2.end());
std::vector<double> Load15_fc3(P_Load_Meas3.begin(), P_Load_Meas3.end());
...
std::vector<double> Load1_fc = repeatStatus(Load15_fc1);   // 1440 
std::vector<double> Load2_fc = repeatStatus(Load15_fc2);   // 1440
std::vector<double> Load3_fc = repeatStatus(Load15_fc3);   // 1440
...
DG1_U1    = slice(DG1_U1);
SoC1      = slice(SoC1);
P_Load_Meas1      = slice(P_Load_Meas1);
P_Load_Meas2      = slice(P_Load_Meas2);
P_Load_Meas3	  = slice(P_Load_Meas3);
...
Load1_fc  = slice(Load1_fc);      //
Load2_fc = slice(Load2_fc);      //
Load3_fc = slice(Load3_fc);      //
...
std::vector<double> Load_used        = Load1_fc;

/* ---- 5.  Thay giá trị dự báo phút ED bằng số đo thực tế ---- */
if (!Load1m_forcast.empty() && !P_Load_Meas1.empty())
Load1m_forcast[0] = P_Load_Meas1[0];   // phút đầu tiên (ED)

if (!Load2m_forcast.empty() && !P_Load_Meas2.empty())
Load2m_forcast[0] = P_Load_Meas2[0];

if (!Load3m_forcast.empty() && !P_Load_Meas3.empty())
Load3m_forcast[0] = P_Load_Meas3[0];
...
for (int j = 0; j < StepNum1m; j++) {
	...
	totalGridBuyCost += P_GridBuyExtra[j] * GridBuyCost1[j] * StepReSol * 1.2;
	totalGridSellCost += -P_GridSellExtra[j] * GridSellCost1[j] * StepReSol * 0.8;
}
//
// ---------------------------------DG Variables-----------------------------------
...
for (size_t i = 0; i < NumDG; i++)
{
	Committed[i] = IloBoolVarArray(env, StepNum1m);
...
for (int b = 0; b < NumBESS; ++b)
{
	...
	for (int t = 1; t < StepNum1m; ++t)
	{
		...
		socDyn += k_dch * P_BESS_dch[b][t - 1];			//  + k_dch·P_dch(t-1)
		socDyn += -k_ch * P_BESS_ch[b][t - 1];			//  − k_ch ·P_ch (t-1)
...
for (int i = 0; i < NumBESS; ++i)
{
	...
	for (int t = 0; t < StepNum1m; ++t)
	{
		...
		model.add(P_BESS_dch[i][t] <= U_BESS_dch[i][t] * PCS_Rate[i]);
		model.add(P_BESS_ch[i][t] <= U_BESS_ch[i][t] * PCS_Rate[i]);
		/* --- 2. P_BESSdch,max = min( … )  --------------------- *
...
for (int i = 0; i < NumBESS; ++i)
{
	...
	const double alpha1 = BattCapa[i] * k1 / StepReSol;
...
//	std::string modelOutFile = Output_path + "/Model_Microgrid_UC.lp";
...
//
//	/* ---- 6.3  Mở file ---- */
//	std::ofstream csv(Output_path + "/Output_Microgrid_UC.csv");
//	if (!csv.is_open())
